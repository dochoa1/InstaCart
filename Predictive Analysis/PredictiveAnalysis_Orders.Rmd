...
(nullCon <- confusionMatrix(nullPredict, test$reordered))
```
...
```{r}
cv <- xgb.cv(data = trainingMatrix, nfold=4, param=params, nrounds=130, early_stopping_rounds=20, print_every_n = 10)
```

```{r}
model <- xgb.train(data = trainingMatrix, param=params, nrounds=50, verbose=FALSE)
