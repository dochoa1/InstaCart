...
Still improving after 100 iterations, need to use more! Marginal improvement tho
```{r}
cv <- xgb.cv(data = trainingMatrix, nfold=4, param=params, nrounds=100, early_stopping_rounds=20, print_every_n = 10)
```

```{r}
model <- xgb.train(data = trainingMatrix, param=params, nrounds=100, verbose=FALSE)
