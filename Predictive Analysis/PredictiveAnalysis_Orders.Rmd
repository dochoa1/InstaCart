...
This feature is going to keep track of how many consecutive orders before the current order has a specific user ordered a specific product. The intution behind this is that if a user has ordered a given product every order of their past n orders, than as n gets larger, we expect this user to order this product again with high likelihood.

Note: This is computationally quite expensive, it will probably be best to move all of the data transformation to a seperate file in /DataPrep and have another, seperate file in /PredictiveAnalysis, concerned with just the model training.
```{r}
