...
trainIndependents <- train %>% 
  select(-reordered, -product_id, -order_id, -user_product.avgCartPosition)

testIndependents <- test %>% 
  select(-reordered,  -product_id, -order_id, -user_product.avgCartPosition)
```

Parameters that we have found to hurt the model more than aid (noise): product_id, order_id, user_product.avgCartPosition
...
Still improving after 60 iterations, need to use more! Marginal improvement tho, could focus on this after adding other features or tuning.
```{r}
cv <- xgb.cv(data = trainingMatrix, nfold=4, param=params, nrounds=40, early_stopping_rounds=8, print_every_n = 10)
```

```{r}
model <- xgb.train(data = trainingMatrix, param=params, nrounds=60, verbose=FALSE)
...
```{r eval=FALSE}
xgb.plot.tree(feature_names=colnames(importance), model=model_linear, n_first_tree=1)
```
