...
library(tidyverse)
library(xgboost)
library(Ckmeans.1d.dp) # Used for XGBoost visuzalization
```
...
```{r}
trainingMatrix <- xgb.DMatrix(as.matrix(sampleTrain %>% select(-reordered)), label = sampleTrain$reordered)
...
test$reordered <- (test$reordered > 0.95) * 1 # 0.95 is random threshold I came up with, experiment with it
```
...
Let picture the overlap in predictions and actual orders
```{r}
crossVal_viz <- crossVal %>%
  filter(order_id == 1232929) %>%
  mutate(predictedToOrder = reordered) %>%
  select(product_id, predictedToOrder)

order_products_viz <- order_products %>%
  filter(order_id == 1232929) %>%
  mutate(actuallyOrder = reordered) %>%
  select(product_id, actuallyOrder) %>%
  full_join(crossVal_viz, by="product_id")

order_products_viz <- replace(order_products_viz,is.na(order_products_viz),0)

ggplot() + geom_point(data=order_products_viz, aes(x=product_id, y=actuallyOrder), color="red", alpha=0.2) + geom_point(data=order_products_viz, aes(x=product_id, y=predictedToOrder), color="blue", alpha=0.2)
```


We can see from this graphic that we were able to correctly predict some of the products that the order would contain (the dots in purple!) There is still a lot of room for improvement!