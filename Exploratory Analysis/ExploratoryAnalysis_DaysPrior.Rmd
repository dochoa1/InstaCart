...
TODO: Talk about interesting findings
```{r}
ggplot(data=head(orders_products_daysSinceProductLastOrdered, 20), aes(x=product_id, y=avg_days_since_product_last_ordered)) + geom_point(alpha=0.05) + geom_text(aes(label=product_name), angle=35)

ggplot(data=tail(orders_products_daysSinceProductLastOrdered, 20), aes(x=product_id, y=avg_days_since_product_last_ordered)) + geom_point(alpha=0.05) + geom_text(aes(label=product_name))
```