...
orders_products_daysSinceProductLastOrdered <-
  orders_products %>%
  group_by(user_id) %>%
  mutate(days_since_product_last_ordered = ave(cumulative_days, product_id, FUN=function(x) c(0, diff(x)))) %>%
  ungroup() %>%
...
TODO: Requirment on number of orders? Too many 1's for top 20/100
```{r}
ggplot(data=head(orders_products_daysSinceProductLastOrdered, 20), aes(x=product_id, y=avg_days_since_product_last_ordered)) + geom_point(alpha=0.05) + geom_text_repel(aes(label=product_name), color="seagreen4") + labs(title="The 20 Products With The Lowest Average Days Since Last Order", x="Product ID", y="Average Days Since Product Was Last Ordered")

ggplot(data=tail(orders_products_daysSinceProductLastOrdered, 20), aes(x=product_id, y=avg_days_since_product_last_ordered)) + geom_point(alpha=0.05) + geom_text_repel(aes(label=product_name), color="seagreen4") + labs(title="The 20 Products With The Highest Average Days Since Last Order", x="Product ID", y="Average Days Since Product Was Last Ordered")
```
